<?php session_start();
	...
	if ($_SERVER["REQUEST_METHOD"] === "POST") {
	  // Check if the form was submitted through the addInventoryBtn button
	  ...
	  //   if (empty($errors)) {
	  //     // Perform additional operations, such as storing data in the database or handling the file upload
	  //     // ...
	  ...
	  // }
		...
		if (empty($errors)) {
	      ...
	      $stmt = $con->prepare("INSERT INTO products (product_name, product_price, product_desc, product_img, stock, added_by, date_stock_update, updated_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");

	      // Bind the parameters and execute the statement
	      $stmt->bind_param("ssssiisi", $productName, $productPrice, $productDesc, $uniqueFileName, $productStock, $added_by, $date_stock_update, $updated_by);
	      ...
	      $con->close();
		    ...
		    $errors[] = "Failed to move the image file.";
		    $response = array("success" => false, "errors" => $errors);
		  }
