<?php session_start();
	...
	$sql = "SELECT product_id, product_name, product_price, product_desc, product_img FROM products WHERE stock != 0";
	...
	if ($result -> num_rows < 1) {
		...
		while ($data = $result->fetch_assoc()) { ?>
			...
			<div class="card proCard" product-name="<?= $data['product_name']; ?>" product-price="<?= $data['product_price']; ?>" product-id="<?= $data['product_id']; ?>" product-img="<?= $data['product_img']; ?>">
